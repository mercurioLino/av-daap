...
class Messenger:
    ...
    async def message(
        ...
        if self._is_quick_reply_message(message):
            ...
            print("ATACHMENTS USER MESSAGE", message["message"])
            text = message["message"]["text"]
        elif self._is_audio_message(message):
            print("ATACHMENTS AUDIO", message["message"]["attachments"])
            ...
            print("FILE", message["message"]["attachments"])
            attachment = message["message"]["attachments"][0]
            text = attachment["payload"]["url"]
        else:
            logger.warning(
